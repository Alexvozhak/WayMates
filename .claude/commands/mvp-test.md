...
## 📝 Pre-Action Declaration (перед каждым тестом)

**ОБЯЗАТЕЛЬНО перед каждым тестом:**

- **Проблема**: [какой сценарий покрываем]
- **Решение**: [как тестируем, что проверяем]
- **Источник**: [правило 4 вопросов, бизнес-логика, edge case]
- **Уверенность**: X% — [почему]
- **Объём**: ~N LOC, Xk/Yk токенов (успею точно / 50/50 / маловероятно / нет)

**Затем AskUserQuestion** с вариантами:
- "Да, пиши тест (Recommended)"
- "Изменить подход"
- "Skip (Coverage Theater)"
- **"Зафиксировать сессию"** — если нужно записать Session Report

**Пример:**
```
Проблема: Проверить penalty при исключении skills из scoring
Решение: Unit тест с excludedSkills содержащими все skills
Источник: Бизнес-правило из design doc (penalty = 0.3)
Уверенность: 95% — правило явное, математика простая
Объём: ~15 LOC, 3k/120k (успею точно)
```
...
| 6   | Тест без Pre-Action заявки     | Сначала обосновать     |
...
- [ ] Pre-Action Declaration для каждого теста
...
### Testing
- **Сделано**: [тесты, TC-XXX, статус]
- **Инсайты**: [какие edge cases нашли, ценность тестов]
- **Решения по месту**: [skip теста, изменение подхода]
- **Ошибки**: [падающие тесты, flaky, fixtures]
- **Для след. сессии**: [какие тесты добавить]
...
- [примеры: "Правило 4 вопросов полезно", "Coverage Theater нужно проверять раньше"]
```
...
1. **Pre-Action Declaration** — обязательно перед каждым тестом (раздел выше)
2. **Session Report** — вести по ходу, фиксировать при <10% контекста
3. **Бизнес-ценность** — тестировать то, что важно для пользователя
4. **Правило 4 вопросов** — применять к каждому тесту
5. **JSDoc обязателен** — документация в коде
6. **Нет coverage theater** — качество важнее процентов
7. **Root cause при падении** — не отмазываться "flaky"
