...
### CLI скрипт — прочитать перед использованием
Использовал несуществующие флаги (--session, --reset) → прочитать скрипт перед использованием. telegram-chat.ts поддерживает только `--start`, `--file`, `--wait-double`.
