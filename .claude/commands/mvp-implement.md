...
### 2.5. Pre-Action Declaration (перед каждым действием)
...
- **Знание кодовой базы (достаточно осмотрелся)**: X% — [почему]
- **Понимание бизнес-логики (как оно устроено)**: X% — [почему]
- **Понимание бизнес-смысла (зачем я это делаю)**: X% — [почему]
- **Объём**: ~N LOC, Xk/Yk токенов (X - сколько предположительно уйдет токенов на этот шаг, Y-сколько осталось от сессии (с учетом отключенного autocompact))

**Затем AskUserQuestion** с вариантами:

- "Да, реализуй (Recommended)"
- "Изменить подход"
- "Показать альтернативу"
- **"Зафиксировать сессию"** — если нужно записать Session Report

**Пример:**

```
Проблема: Per-user rate limiting для изоляции пользователей
Решение: Map<UserId, Bottleneck>, on-demand создание
Источник: context7 /bottleneck/docs + додумал Map (не WeakMap для долгосрочной изоляции)
Уверенность: 90% — документация покрывает, паттерн простой
Объём: ~25 LOC, 8k/150k (успею точно)
```
...
| 10  | Код без Pre-Action заявки | Сначала описать план   |

---

## ✅ Критерии завершения реализации

- [ ] Pre-Action Declaration для каждого действия
- [ ] Session Report ведётся и зафиксирован
...
export async function rateLimitedCall<T>(userId: UserId, operation: string, fn: () => Promise<T>): Promise<T> {
  const userLimiter = getUserLimiter(userId);

  return globalLimiter.schedule(() => userLimiter.schedule(fn));
}
```
...
## 📊 Session Report (вести по ходу)

**Файл**: `docs/sessions/mvp-session-[feature-name].md`

**Графы** (обновлять по ходу):
...
- **Сделано**: [файлы, LOC]
- **Инсайты**: [что оказалось сложнее/проще про код/архитектуру]
- **Решения по месту**: [отступления от дизайна, рефакторинг]
- **Ошибки**: [ESLint, type conflicts, integration]
- **Для след. сессии**: [что доработать в коде]

### Общие инсайты (без привязки к фазе)

- [что помогло/мешало в ПРОЦЕССЕ работы, какие шаги промпта были полезны/бесполезны]

### Улучшения промпта (рекомендации для MVP команд)

- [что добавить/убрать/изменить в промптах, чтобы работать эффективнее]
- [примеры: "Pre-Action помогает", "ESLint надо проверять до кода"]
```

**Триггеры фиксации**:

- В каждом Pre-Action AskUserQuestion → вариант **"Зафиксировать сессию"**
- **При <10% контекста** → автоматически предложить зафиксировать

---

## ⚠️ Важно

1. **Pre-Action Declaration** — обязательно перед каждым действием (раздел 2.5)
2. **Session Report** — вести по ходу работы, зафиксировать при <10% контекста
3. **Строго по дизайну** — не импровизировать
4. **ESLint first** — учитывать constraints при написании
5. **Checkpoint после блока** — показать результат
6. **Не спамить проверками** — lint/tsc после логического блока
7. **Готовность к тестам** — код должен быть testable
