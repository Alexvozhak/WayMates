...
describe("Cypher Queries Integration Tests", () => {
  ...
  beforeAll(() => {
    driver = createDriver();
    manager = new PersistenceManager(driver);
  });
  ...
  describe("Upserts.CREATE_CONTEXT", () => {
    test("creates user and context with basic properties", async () => {
      const { userId, context, contextId } = await upsertAndGetContext(
        driver,
        manager,
        "USER_001",
        0
    ...
    test("duplicates context fields as properties for fast search", async () => {
      const { context: inputContext, contextId: firstContextId } =
        await upsertAndGetContext(driver, manager, "USER_001", 0);
    ...
    test("creates position relationship", async () => {
      const { context, contextId } = await upsertAndGetContext(
        driver,
        manager,
        "USER_001",
        1
    ...
    async function updateAndAssert(aspect: "properties" | "skills") {
      ...
      await upsertAndGetContext(driver, manager, "USER_002", 0);
      ...
      const res = await manager.upsertContexts({
    ...
    test("creates temporal links between contexts", async () => {
      ...
      const result1 = await manager.upsertContexts({
        user_id: testData.user_id,
      ...
      const result2 = await manager.upsertContexts({
        user_id: testData.user_id,
    ...
    describe("Upserts.CREATE_TRAIL", () => {
      test("creates trail with user relationship", async () => {
        ...
        const { userId, contextId: fromContextId } = await upsertAndGetContext(
          driver,
          manager,
          "USER_002",
          0
        );
        const { contextId: toContextId } = await upsertAndGetContext(
          driver,
          manager,
          "USER_002",
          1
        ...
        // Create trail
        const result = await manager.upsertTrails({
          user_id: userId,
          trails: [trailData],
        });
    ...
    describe("Negative cases", () => {
      ...
      test("idempotent upsert does not duplicate nodes", async () => {
        const { userId, context } = await upsertAndGetContext(
          driver,
          manager,
          "USER_001",
          0
