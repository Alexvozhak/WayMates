 ...
 */
function renderMetricsTable(metrics: SimilarityMetrics[], trajectories: ProcessedTrajectory[]): string {
...
export function generateChartHtml(data: ChartPageData): string {
  ...
  const controls = buildControls(fields, selectedFields, trajectories);
  const metricsTable = metrics.length > 0 ? renderMetricsTable(metrics, trajectories) : "";
